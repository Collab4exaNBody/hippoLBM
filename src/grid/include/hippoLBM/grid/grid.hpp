...
namespace hippoLBM
{
	struct LBMGrid
	{
		...
		LBMPoint offset; /*** offset of the real grid */
		int ghost_layer = 2; /*** default is 2 for dem + lbm */
		double dx; /*** distance between two points   */

		LBMGrid() {};

		LBMGrid(box<DIM>& b, LBMPoint& o, const int g, double d) : bx(b), offset(o), ghost_layer(g), dx(d)
		{
		...
		inline void set_offset(LBMPoint& o) {offset = o;}
		inline void set_offset(LBMPoint&& o) {offset = o;}
		...
		template<Area A, Traversal Tr>
			ONIKA_HOST_DEVICE_FUNC inline bool contains(LBMPoint& p) const
			{
				for (int dim = 0; dim < DIM ; dim++)
				{
		...
		ONIKA_HOST_DEVICE_FUNC inline bool is_defined(LBMPoint& p) const
		{
			for (int dim = 0; dim < DIM ; dim++)
			{
		...
		ONIKA_HOST_DEVICE_FUNC inline bool is_local(LBMPoint& p) const
		{
			for(int dim = 0 ; dim < DIM ; dim++)
			{
		...
		ONIKA_HOST_DEVICE_FUNC inline bool is_global(LBMPoint& p) 
		{
			LBMPoint local = p + offset;
			return is_local(local);
		}

		/*** @brief convert a point to A area. */
		template<Area A, bool Check=false>
			ONIKA_HOST_DEVICE_FUNC inline LBMPoint convert(int x, int y, int z) const
			{
				LBMPoint res = {x, y, z};
		...
		template<Area A, bool Check=false>
			ONIKA_HOST_DEVICE_FUNC inline LBMPoint convert(LBMPoint p) const
			{
		...
		ONIKA_HOST_DEVICE_FUNC int operator()(LBMPoint& p) const
		{
