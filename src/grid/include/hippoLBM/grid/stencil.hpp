...
ONIKA_HOST_DEVICE_FUNC void stencil_for_each_impl(F&& f, std::integer_sequence<int, Is...>) {
...
ONIKA_HOST_DEVICE_FUNC void stencil_for_each(F&& f) {
  stencil_for_each_impl<LBMScheme, Start, End, Step>(std::forward<F>(f), make_filtered_sequence<Start, End, Step>{});
}
