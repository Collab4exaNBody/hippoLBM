 ...
 * @param periods An array indicating whether each dimension has periodic boundary conditions (1 for periodic, 0 for
 * non-periodic).
 * @param ndims An array of the number of dimensions in each dimension.
 * @return A tuple containing a boolean flag indicating whether the communication coordinate is valid and the computed
 * communication coordinate.
 */
inline std::tuple<bool, int3d> build_comm(const int3d& relative_pos, std::span<int> coord, std::span<bool> periods,
                                          std::span<int> ndims) {
  constexpr int DIM = 3;
  int3d coord_neig;
...
/** @brief Fix a box with periodic boundary conditions.
 * @param shift The shift vector indicating the direction of communication.
 * @param g The grid object containing the real box and ghost layer information.
 * @return The adjusted box that accounts for periodic boundary conditions.
 */
