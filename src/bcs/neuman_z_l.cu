...
class NeumannZL : public OperatorNode {
  ...
  ADD_SLOT(readVec3, U, INPUT, REQUIRED,
           DocString{"Prescribed velocity at the boundary (z = lz), enforcing the Neumann condition."});

 public:
  inline std::string documentation() const final {
    return R"EOF(
    This operator enforces a Neumann boundary condition at z = lz in an LBM simulation. 
