...
class LogLBM : public OperatorNode {
 ...
 public:
  ...
  ADD_SLOT(time_point, previous_time, PRIVATE, time_point(), DocString{"The time point of the previous log entry."});
  ADD_SLOT(long, previous_step, PRIVATE, 0, DocString{"The timestep of the previous log entry."});

  inline std::string documentation() const final {
    return R"EOF(
    This operator logs the current state of the LBM simulation,
...
// === register factories ===
ONIKA_AUTORUN_INIT() { OperatorNodeFactory::instance()->register_factory("log", make_variant_operator<LogLBM>); }
}  // namespace hippoLBM
