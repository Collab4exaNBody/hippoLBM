...
namespace hipoLBM
{
	void LBM::write_pvtr( std::string basedir,  std::string basename, size_t number_of_files, grid<3>& Grid)
	{
		...
		if(rank == 0)
		{
			...
			{
				std::string subfile = basename + "/" + basename + "_" + std::to_string(i) + ".vtr" ;
  ...
  template<typename GridDataQ>
	void LBM::write_vtr( std::string name, grid<3>& Grid, GridDataQ& data)
	{
		name = name + ".vtr";
		std::ofstream outFile(name);
		if (!outFile) {
    ...
    auto [traversal_ptr, traversal_size] = traversal.get_data<Tr>();
		...
		parallel_for_id_data(traversal_ptr, traversal_size, writter, outFile, onika::cuda::vector_data(data.m0));
		outFile << std::endl;
		outFile << "          </DataArray>"  << std::endl;
		outFile << "          <DataArray type=\"Float32\" Name=\"U\" format=\"ascii\" NumberOfComponents=\"3\">" << std::endl;
		parallel_for_xyz_core<L, Tr>(writter_vec3d, outFile, onika::cuda::vector_data(data.m1));
		outFile << std::endl;
		outFile << "          </DataArray>"  << std::endl;
		outFile << "          <DataArray type=\"Float32\" Name=\"OBST\" format=\"ascii\">" << std::endl;
		parallel_for_id_data(traversal_ptr, traversal_size, writter, outFile, onika::cuda::vector_data(data.obst));
		outFile << std::endl;
		outFile << "          </DataArray>"  << std::endl;
		outFile << "      </PointData>"  << std::endl;
