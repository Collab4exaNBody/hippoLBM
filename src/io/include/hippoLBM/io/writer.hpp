...
struct UWriter {
  const int* const obst_;       ///< Pointer to the obstacle field.
  const double ratio_dx_dtLB_;  ///< The ratio of grid spacing to LBM time step.
...
struct PressionWriter {
  const int* const obst_;               ///< Pointer to the obstacle field.
  const double c_c_avg_rho_div_three_;  ///< A constant factor for pressure calculation.
  inline double operator()(const int idx, const double& m0) const {
    if (obst_[idx] == FLUIDE_) {
      return c_c_avg_rho_div_three_ * (m0 - 1);
    }
    return 0;
  }
};
...
struct write_file {
  Func func_;  ///< The function to compute the data to be written.
  template <typename T>
...
struct WriterExternalData {
  ...
  uint64_t num_elements_;  // The number of elements in the data to be written.
  inline void operator()(int idx, std::stringstream& output, double* const input_data) const {
    for (int i = 0; i < num_components_; i++) {
      double tmp;
#ifdef WFAOS
      tmp = input_data[idx * num_components_ + i];
#else
      tmp = input_data[num_elements_ * i + idx];
#endif
      output << (float)tmp << " ";
    }
  }
...
template <typename Func>
struct WriteVec3d {
  Func func_;  // The function to compute the 3D vector data to be written.
  Box3D b_;    // The computational box representing the local domain, including ghost layers.
  ...
  inline void operator()(const int x, const int y, const int z, std::stringstream& output,
    ...
    const int idx = b_(x, y, z);
    onika::math::Vec3d tmp = func_(idx, ptr[idx]);
  ...
  inline void operator()(const int x, const int y, const int z, std::stringstream& output,
    ...
    const int idx = b_(x, y, z);
    onika::math::Vec3d tmp = func_(idx, WF);
    output << (float)tmp.x << " " << (float)tmp.y << " " << (float)tmp.z << " ";
  }
};
}  // namespace hippoLBM
