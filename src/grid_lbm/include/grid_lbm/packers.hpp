...
#pragma omp parallel for collapse(3)
      ...
      inline void operator()(WrapperF<Components>& dst, const WrapperF<Components>& src, const box<DIM>& dst_box, const box<DIM>& mesh_box) const
      {
        static_assert(DIM == 3);
        for(int dim = 0 ; dim < DIM ; dim++) 
        {
...
#pragma omp parallel for collapse(2)
        for(int z = inf[2] ; z <= sup[2] ; z++)
          for(int y = inf[1] ; y <= sup[1] ; y++)
          {
            ...
            copyTo<Components>(dst, dst_idx, src, src_idx, dst_box.get_length(0));
          }
      }

      ONIKA_HOST_DEVICE_FUNC inline void operator()(int x, int y, int z, WrapperF<Components>& dst, const WrapperF<Components>& src, const box<DIM>& dst_box, const box<DIM>& mesh_box) const
      {
        const auto& inf = dst_box.inf;
        const int dst_idx = compute_idx<DIM>(dst_box, x - inf[0], y-inf[1], z-inf[2]);
        const int src_idx = compute_idx<DIM>(mesh_box, x, y, z);
...
#pragma omp parallel for collapse(3)
      ...
      inline void operator()(WrapperF<Components>& dst, WrapperF<Components>& src, const box<DIM>& src_box, const box<DIM>& mesh_box) const
      {
        static_assert(DIM == 3);
        for(int dim = 0 ; dim < DIM ; dim++) 
        {
...
#pragma omp parallel for collapse(2)
        for(int z = inf[2] ; z <= sup[2] ; z++)
          for(int y = inf[1] ; y <= sup[1] ; y++)
          {
            ...
            copyTo<Components>(dst, dst_idx, src, src_idx, src_box.get_length(0));
          }
      }

      ONIKA_HOST_DEVICE_FUNC inline void operator()(int x, int y, int z, WrapperF<Components>& dst, WrapperF<Components>& src, const box<DIM>& src_box, const box<DIM>& mesh_box) const
      {
        const auto& inf = src_box.inf;
        const int dst_idx = compute_idx(mesh_box, x , y , z);
        const int src_idx = compute_idx(src_box, x - inf[0], y - inf[1], z - inf[2]);
        copyTo<Components>(dst, dst_idx, src, src_idx, 1);
      }
    };
}
